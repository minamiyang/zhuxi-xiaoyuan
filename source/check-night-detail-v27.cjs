...
(async()=>{const phase=process.argv[2]||'after',dir='verification/月夜水纹精修_20260909';
...
await p.goto('http://127.0.0.1:8770/preview/?v=27-'+phase,{waitUntil:'domcontentloaded'});await p.waitForFunction(()=>window.sceneReady,null,{timeout:120000});
for(const h of phase==='before'?[21]:[5.8,12,17.3,18.8,21,2]){
...
if(phase!=='before'){
await p.evaluate(()=>viewer.seekFrame({seconds:9,hour:21,camera:{position:[3.8,7.6,13],target:[3.1,.9,.0],zoom:2.25}}));
await p.screenshot({path:`${dir}/${phase}-21-front.png`});
