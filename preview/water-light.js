...
`;
export function refractStreamBed(scene,time,sunEnergy,lightDirection,moon){
 let count=0;
 ...
 for(const m of Array.isArray(o.material)?o.material:[o.material]){
  ...
  m.onBeforeCompile=shader=>{previous.call(m,shader);shader.uniforms.uWaterTime=time;shader.uniforms.uWaterSun=sunEnergy;shader.uniforms.uWaterLightDir=lightDirection;shader.uniforms.uWaterMoon=moon.energy;shader.uniforms.uWaterMoonDir=moon.direction;shader.uniforms.uWaterMoonColor=moon.color;
   shader.vertexShader='varying vec3 vRefractedP;\n'+shader.vertexShader;
   shader.vertexShader=shader.vertexShader.replace('#include <project_vertex>','#include <project_vertex>\nvRefractedP=(modelMatrix*vec4(transformed,1.)).xyz;');
   shader.fragmentShader='varying vec3 vRefractedP;uniform float uWaterTime;uniform float uWaterSun;uniform vec3 uWaterLightDir;uniform float uWaterMoon;uniform vec3 uWaterMoonDir,uWaterMoonColor;\n'+causticGLSL+shader.fragmentShader;
   ...
   shader.fragmentShader=shader.fragmentShader.replace('#include <lights_fragment_end>',`#include <lights_fragment_end>
    if(underwater>.001){
     ...
     // Moonlight traverses the same moving wave lens field as sunlight.
     // Geometry and the moon shadow keep the bridge underside dark.
     if(uWaterMoon>.001){
      vec3 moonRay=refract(-normalize(uWaterMoonDir),vec3(0.,1.,0.),1./1.333);
      vec2 moonOffset=moonRay.xz/max(.2,-moonRay.y)*max(0.,.36-vRefractedP.y);
      vec2 moonP=vRefractedP.xz-moonOffset;
      float moonFold=.78*waterCaustic(moonP*1.3,uWaterTime)+.22*waterCaustic(moonP*2.15+vec2(1.7,-.8),uWaterTime);
      float glimmer=.6+.4*smoothstep(-.6,.8,sin(moonP.x*2.1+moonP.y*1.7-uWaterTime*.25));
      float streamMoonShadow=1.;
      #ifdef USE_SHADOWMAP
      #if NUM_DIR_LIGHT_SHADOWS > 1
       DirectionalLightShadow waterMS=directionalLightShadows[1];
       streamMoonShadow=getShadow(directionalShadowMap[1],waterMS.shadowMapSize,waterMS.shadowIntensity,waterMS.shadowBias,waterMS.shadowRadius,vDirectionalShadowCoord[1]);
      #endif
      #endif
      float bedFacing=max(0.,dot(normal,normalize((viewMatrix*vec4(uWaterMoonDir,0.)).xyz)));
      reflectedLight.directDiffuse+=uWaterMoonColor*moonFold*underwater*uWaterMoon*.26*glimmer*streamMoonShadow*bedFacing;
     }
    }
   `);
  };m.customProgramCacheKey=()=>cache+'-lit-stream-v27';m.needsUpdate=true;
 }
 });return count;
}
