...
export function sampleDay(hour){
 ...
 return {hour:h,altitude,day,high,warm,golden,sunrise,sunset,sunPower,lamps,moonPower:.64*(1-day),fogBuild:smooth(4.6,6.2,h),fogClear:smooth(6.2,8.5,h),fog:.16*Math.exp(-(((h-6.2)/1.13)**2))*(1-smooth(7.3,8.5,h)),sunPosition,skySunPosition,exposure:mix(.97,1.03,day)};
}
// Shared world-space growth/erosion: moist pockets form first, upper/open areas clear first.
const FOG_EVOLUTION=`
uniform float uFogBuild;uniform float uFogClear;
float fogEvolution(vec3 p){
 float creek=exp(-pow((p.x-3.55-.48*sin(-p.z*.58))/1.6,2.));
 float roots=exp(-pow((p.x+3.6)/2.,2.));
 float pocket=clamp(.5+.22*sin(p.x*1.15+p.z*.8-uWeatherTime*.035)+.16*sin(p.z*1.8-p.x*.65+uWeatherTime*.027),0.,1.);
 float rank=clamp(.2+.26*(1.-max(creek,roots))+.24*pocket+.3*clamp((p.y-.4)/1.7,0.,1.),0.,1.);
 float forming=smoothstep(rank*.48,rank*.48+.48,uFogBuild);
 float clearing=1.-smoothstep((1.-rank)*.55,(1.-rank)*.55+.45,uFogClear);
 return forming*clearing;
}`;
const PALETTE=[
...
export function createDaylight({scene,renderer,sun,fill,hemi,ground,noAO,camera}){
 let running=true,duration=240,anchorHour=5.3,anchorSeconds=0,lastHour=5.3,state=sampleDay(5.3);
 const fogUniforms={uGolden:{value:0},uDawnFog:{value:0},uFogBuild:{value:0},uFogClear:{value:0},uFogTint:{value:new T.Color()},uWeatherTime:{value:0},uFogViewDir:{value:new T.Vector3()}};
 ...
 void main(){
  ...
  gl_FragColor=vec4(uFogTint,envelope*max(.08,wisps)*height*fogEvolution(p)*uDawnFog*(.48*11./48.));
 }`});
 ...
 float fogDepth=0.;
 if(t1>t0){for(int i=0;i<6;i++){vec3 q=origin+rd*mix(t0,t1,(float(i)+.5)/6.);float height=exp(-max(0.,q.y-.38)*1.85);float bank=.72+.28*exp(-pow((q.x-3.6)/2.,2.));float drift=.76+.15*sin(q.x*1.25+q.z*.8-uWeatherTime*.055)+.09*sin(q.x*3.2-q.z*1.4+uWeatherTime*.038);fogDepth+=height*bank*drift*fogEvolution(q);}fogDepth*=(t1-t0)/6.;}
 gl_FragColor.rgb=mix(gl_FragColor.rgb,uFogTint,1.-exp(-fogDepth*uDawnFog));
 }`);
 };m.customProgramCacheKey=()=>key+'-height-mist-v25-'+thinLeaf;m.needsUpdate=true;
 }});}
 ...
 ground.material.color.copy(bg).multiplyScalar(mix(.65,.36,s.day));ground.material.emissive.copy(bg).multiplyScalar(mix(.82,.42,s.day));fogUniforms.uGolden.value=s.golden;fogUniforms.uDawnFog.value=s.fog;fogUniforms.uFogBuild.value=s.fogBuild;fogUniforms.uFogClear.value=s.fogClear;fogUniforms.uFogTint.value.copy(sky).lerp(new T.Color('#f3e2c5'),.44);fogUniforms.uWeatherTime.value=seconds;camera.getWorldDirection(fogUniforms.uFogViewDir.value);fogObjects.forEach(o=>o.visible=s.fog>.0001);
