...
export function createSoundscape(){
 ...
 for(const [id,title]of Object.entries(titles)){const gain=ctx.createGain();gain.gain.value=0;gain.connect(master);tracks.set(id,{id,title,gain,target:0,buffer:null,normalization:1,source:null,loaded:false,timer:0,oneShot:id==='rooster'||id==='dog',pending:false,active:false,playCount:0});}
 ...
 function ui(){const blocked=ctx.state!=='running';button.textContent=muted?'音效已关闭':blocked?'音效待播放':'音效已开启';button.setAttribute('aria-pressed',String(!muted));button.setAttribute('aria-label',muted?'开启环境音效':blocked?'音效已设为开启，等待浏览器允许播放':'关闭环境音效');const active=[...tracks.values()].filter(t=>t.target>.11&&t.loaded).sort((a,b)=>b.target-a.target).slice(0,3).map(t=>t.title).join(' · ');const text=muted?'':blocked?'声音已设为开启，等待浏览器允许播放':active?'正在听 · '+active:failures.length?'部分音效暂未载入':'';if(text!==lastLabel){label.textContent=text;lastLabel=text;}}
 ...
 document.addEventListener('pointerdown',gesture);document.addEventListener('keydown',gesture);document.addEventListener('click',gesture);document.addEventListener('touchend',gesture);document.getElementById('music-audio').addEventListener('playing',unlock);
 button.onclick=()=>{if(ctx.state!=='running'&&!muted){unlock();return;}muted=!muted;if(muted){tracks.get('dog').pending=false;}else if(nightEligible)dogNext=ctx.currentTime+3;envelope(master.gain,muted?0:AMBIENCE_LEVEL,.14);if(!muted)unlock();ui();};
 ...
 return {ready,update(input){lastInput=input;dawn(input);
  ...
  if(eligible!==nightEligible){nightEligible=eligible;dogNext=eligible?ctx.currentTime+3:Infinity;const dog=tracks.get('dog');dog.pending=false;if(!eligible){dog.target=0;envelope(dog.gain.gain,0,.28);}}
  const mix=soundMix(input);for(const t of tracks.values()){if(t.oneShot)continue;const next=mix[t.id]||0;if(Math.abs(next-t.target)>.002){t.target=next;envelope(t.gain.gain,next*t.normalization);}}ui();},get state(){return {context:ctx.state,muted,nightEligible,dogNext,failures,input:lastInput,tracks:[...tracks.values()].map(t=>({id:t.id,target:t.target,gain:t.gain.gain.value,loaded:t.loaded,duration:t.buffer?.duration,rms:t.rms,peak:t.peak,normalization:t.normalization,oneShot:t.oneShot,active:t.active,pending:t.pending,playCount:t.playCount}))};},dispose(){disposed=true;clearInterval(timer);document.removeEventListener('pointerdown',gesture);document.removeEventListener('keydown',gesture);document.removeEventListener('click',gesture);document.removeEventListener('touchend',gesture);document.getElementById('music-audio').removeEventListener('playing',unlock);document.removeEventListener('visibilitychange',visibility);ctx.close();}};
}
