...
export function createStorySky({scene,camera,controls,ground,noAO,renderer}){
 ...
 varying vec2 vUv;uniform vec3 skyBase,skyTop,skyHorizon;uniform float dusk,sunVis,moonVis,starVis,sunWarm,dawn;uniform float day,warm,clock,time,aspect,detail,viewShift,sunX;uniform vec2 moonP;
 ...
 vec4 cloud(vec2 p,float size,float seed){
  ...
  vec3 lightDir=normalize(mix(vec3(-.45,.8,.6),vec3(mix(1.,-1.,dawn),.15,.35),dusk));
  for(int k=0;k<11;k++){
 ...
 float stars(vec2 p,float scale,float seed){
  ...
  return (core+glow)*step(.85,h)*twinkle;
 }
 void main(){
 ...
 vec2 sunP=vec2(sunX*aspect,mix(.73,.76,scale)+.18*scale*sin((clock-6.)*3.14159265/12.)-.12*smoothstep(16.,18.6,clock)-.11*dawn*(1.-smoothstep(6.,8.,clock)));
 ...
 evening=mix(evening,evening*vec3(1.02,1.04,1.11),dawn*.45);
 ...
 // Leave the aureole unobstructed; narrow procedural veils read as horizontal stripes.
 ...
 function update(d,seconds){
  ...
  u.dawn.value=sm(5.2,6.0,h)*(1.-sm(6.6,8.5,h));
  u.dusk.value=Math.max(sm(15.5,17.35,h)*(1.-sm(18.35,19.65,h)),u.dawn.value*.86);
  u.sunWarm.value=Math.max(d.warm*.45,sm(15.5,17.35,h),u.dawn.value*.9);
  ...
  lightP.set(...(d.skySunPosition||d.sunPosition)).project(camera);u.sunX.value=T.MathUtils.clamp(.5+lightP.x*.25-.10*(1.-T.MathUtils.smoothstep(d.hour,8,11)),.12,.85);
 }
 return {mesh,update,get state(){return {day:u.day.value,detail:u.detail.value,sunX:u.sunX.value,moon:u.moonP.value.toArray(),dusk:u.dusk.value,dawn:u.dawn.value,sunVisible:u.sunVis.value,moonVisible:u.moonVis.value,starsVisible:u.starVis.value,seconds:u.time.value};},dispose(){mesh.geometry.dispose();mat.dispose();}};
}
