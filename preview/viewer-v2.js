...
import {createStorySky} from './sky-v17.js';
...
import {refineDetails,createSupper} from './details-v14.js';
...
function load(){if(loading||loaded)return;loading=true;status.textContent='正在展开这座小院…';new GLTFLoader().setDRACOLoader(new DRACOLoader().setDecoderPath('https://cdn.jsdelivr.net/npm/three@0.180.0/examples/jsm/libs/draco/gltf/')).load('./竹溪小院_动态.glb',g=>{model=g.scene;
 ...
 });scene.add(model);refineGround(model);model.userData.detailRefinement=refineDetails(model);const supper=createSupper(model);scene.add(supper);daylight.patch(supper);
...
window.addEventListener('resize',resize);document.addEventListener('visibilitychange',()=>{lastNow=0;if(document.hidden){if(raf)cancelAnimationFrame(raf);raf=0;}else{dirty=true;request();}});renderer.domElement.addEventListener('webglcontextlost',e=>{e.preventDefault();status.textContent='图形上下文暂时中断，请刷新恢复。';});window.addEventListener('pagehide',()=>{disposed=true;cancelAnimationFrame(raf);controls.dispose();soundscape.dispose();composer.dispose();stream?.dispose();storySky.dispose();daylight.dispose();env.dispose();renderer.dispose();});
window.viewer={setMode,go,setPlaying,lesson,music,soundscape,storySky,
