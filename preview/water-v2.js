...
export function createStream(renderer,scene,camera,sun){
 ...
 const waterSun={value:1};const refractedMaterials=refractStreamBed(scene,uniforms.uTime,waterSun,uniforms.sunDir,{energy:uniforms.moonEnergy,direction:uniforms.moonDir,color:uniforms.moonColor});
 ...
 // Two separate light responses: a fading sun must not erase moon or sky reflection.
  ...
  float moonSpec=pow(moonDot,90.)*2.4+pow(moonDot,18.)*.38;
  vec3 scatter=waterColor*(.70+.30*shadow)*daylightGain;
  float wave=sin(p.y*22.-p.x*6.-t*1.15+1.3*sin(p.x*4.7+p.y*3.1+t*.28));
  float crest=pow(max(0.,wave),12.);
  ...
  // The same moving facets catch each source's own radiance throughout the day.
  // A broad rough-water lobe remains legible from the diorama's elevated camera;
  // sharp reflection peaks still depend on the normal, light and viewing direction.
  vec3 skyRadiance=mix(skyTop,skyHorizon,.58);
  float skyPower=clamp(dot(skyRadiance,vec3(.2126,.7152,.0722)),0.,1.);
  vec3 skyTint=skyRadiance/max(.001,max(skyRadiance.r,max(skyRadiance.g,skyRadiance.b)));
  float sourceBlend=smoothstep(.02,.3,solarEnergy);
  float sourceShadow=mix(moonShadow,shadow,sourceBlend);
  float sunLobe=pow(solarDot,5.);
  float solarGlitter=crest*breakup*(.12+.88*sunLobe)*.46;
  // Moonlit detail comes from the same curved light field as daytime, beneath
  // the surface. Keep only a soft view-dependent reflection above it.
  float skyGlitter=(skySheen*.18+ripple*.52*sourceBlend)*sqrt(skyPower)*(.25+.75*sourceShadow);
  ...
  vec3 color=(scatter*absorption*(1.-reflectionWeight)+refl*reflectionWeight)
    +skyTint*skyGlitter
    +sunColor*solarGlitter*solarEnergy*shadow
    +sunColor*spec*solarEnergy*shadow*.4
    +moonColor*(moonSpec*.075*(.5+.5*breakup))*moonEnergy*moonShadow;
