...
export function createHen(){
 ...
 const brown=standard('#995c2d'),gold=standard('#b97a3c'),wingmat=standard('#996335'),dark=standard('#493b28'),red=standard('#a93225',.64),legmat=standard('#9e7946'),horn=standard('#b89a65',.57),black=standard('#17100b',.24),iris=standard('#a77932',.48);
 ...
 const colors=['#966238','#9b673c','#a06b3f','#996339','#a46d41','#9d663b'];
 ...
 const head=new T.Group();head.name='头部 · 刚性关节';root.add(head);head.scale.setScalar(.96);ellipsoid(V(0,0,0),V(.083,.09,.098),gold,head);ellipsoid(V(0,-.032,.055),V(.065,.059,.065),gold,head);
 ...
 let k=0;for(let row=0;row<10;row++){const u=.035+.9*row/9,{c,tangent,x,v}=frame(u),r=radius(u)+.002;for(let j=0;j<16;j++){const a=(j+(row%2)*.5)/16*Math.PI*2,normal=x.clone().multiplyScalar(Math.cos(a)).addScaledVector(v,Math.sin(a));put(neckFeathers,k++,c.clone().addScaledVector(normal,r),normal,tangent.clone().negate(),.039,.068,row%3===0?'#b0804c':'#a97a46');}}neckFeathers.instanceMatrix.needsUpdate=true;neckFeathers.instanceColor.needsUpdate=true;
 }
