...
export function createStream(renderer,scene,camera,sun){
 ...
 // Two separate light responses: a fading sun must not erase moon or sky reflection.
  ...
  float moonLobe=pow(moonDot,5.);
  float solarGlitter=crest*breakup*(.12+.88*sunLobe)*.46;
  float lunarGlitter=crest*breakup*(.15+.85*moonLobe)*1.05;
  float skyGlitter=(skySheen*.18+ripple*.52)*sqrt(skyPower)*(.25+.75*sourceShadow);
  ...
  vec3 color=(scatter*absorption*(1.-reflectionWeight)+refl*reflectionWeight)
    ...
    +moonColor*(moonSpec*.11*(.25+.75*breakup)+lunarGlitter)*moonEnergy*moonShadow;
