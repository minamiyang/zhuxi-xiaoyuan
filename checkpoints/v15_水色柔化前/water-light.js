...
`;
export function refractStreamBed(scene,time,sunEnergy){
 let count=0;
 ...
 for(const m of Array.isArray(o.material)?o.material:[o.material]){
  ...
  m.onBeforeCompile=shader=>{previous.call(m,shader);shader.uniforms.uWaterTime=time;shader.uniforms.uWaterSun=sunEnergy;
   shader.vertexShader='varying vec3 vRefractedP;\n'+shader.vertexShader;
   shader.vertexShader=shader.vertexShader.replace('#include <project_vertex>','#include <project_vertex>\nvRefractedP=(modelMatrix*vec4(transformed,1.)).xyz;');
   shader.fragmentShader='varying vec3 vRefractedP;uniform float uWaterTime;uniform float uWaterSun;\n'+causticGLSL+shader.fragmentShader;
   shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
    ...
    diffuseColor.rgb=mix(diffuseColor.rgb,bedValue*vec3(.48,.83,1.16),underwater*.86);
   `);
   shader.fragmentShader=shader.fragmentShader.replace('#include <lights_fragment_end>',`#include <lights_fragment_end>
    if(underwater>.001){
     float caustic=waterCaustic(vRefractedP.xz+vRefractedP.y*.24,uWaterTime);
     reflectedLight.directDiffuse*=1.+caustic*underwater*uWaterSun*2.1;
    }
   `);
  };m.customProgramCacheKey=()=>cache+'-blue-stream-v15';m.needsUpdate=true;
 }
 });return count;
}
