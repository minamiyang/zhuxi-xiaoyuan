...
export function createStream(renderer,scene,camera,sun){
 ...
 #endif
 #endif
 vec3 halfDir=normalize(sunDir+e);float spec=pow(max(dot(n,halfDir),0.),110.)*5.2;
  vec3 scatter=waterColor*(.70+.30*shadow)*daylightGain;
  // Broken silver crests from broad sky reflections, separated from the bed caustics.
  ...
  refl*=mix(.40,1.,daylightGain);
  float skySheen=pow(smoothstep(.18,.85,ray.y),3.)*.12;
  vec3 color=mix(scatter,refl,clamp(.28+fresnel*.85,.28,.88))
    +vec3(.65,.82,1.)*(skySheen+ripple*(.22+.78*shadow))*daylightGain
    +keyColor*spec*mix(.22,1.,daylightGain)*shadow*daylightGain;
  float center=3.55+.48*sin(-p.y*.58)+.15*cos(-p.y*1.2);
  float depth=1.-smoothstep(.16,1.05,abs(p.x-center));
  gl_FragColor=vec4(color,.43+.09*depth+fresnel*.38);
 }`});
 ...
 return {water,reflection,update,excluded,refractedMaterials,setLighting(d){waterSun.value=Math.min(1,d.sunPower/2.2);uniforms.daylightGain.value=.045+.955*d.day;uniforms.keyIsMoon.value=d.sunPower>.01?0:1;uniforms.keyColor.value.copy(d.keyColor);uniforms.sunDir.value.copy(d.keyDirection);uniforms.skyTop.value.copy(d.sky).multiplyScalar(.5);uniforms.skyHorizon.value.copy(d.horizon);uniforms.reflectionBG.value.copy(scene.background);},dirty(){dirty=true;},dispose(){reflection.dispose();geometry.dispose();mat.dispose();}};
}
export function createWakes(scene,ducks){
 const wakes=ducks.map((duck,i)=>{const group=new THREE.Group();group.renderOrder=4;const rings=[];for(let j=0;j<3;j++){const g=new THREE.RingGeometry(.97,1,80);g.rotateX(-Math.PI/2);const m=new THREE.MeshBasicMaterial({color:0xcbdfee,transparent:true,opacity:.12,depthWrite:false,side:THREE.DoubleSide});const r=new THREE.Mesh(g,m);group.add(r);rings.push(r);}scene.add(group);return {group,rings};});
