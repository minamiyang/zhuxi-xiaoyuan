...
export function createStorySky({scene,camera,controls,ground,noAO,renderer}){
 const u={skyBase:{value:new T.Color()},skyTop:{value:new T.Color()},day:{value:1},warm:{value:0},clock:{value:0},time:{value:0},aspect:{value:1},detail:{value:1},viewShift:{value:0},sunX:{value:.25},moonP:{value:new T.Vector2(.8,.8)}};
 ...
 varying vec2 vUv;uniform vec3 skyBase,skyTop;uniform float day,warm,clock,time,aspect,detail,viewShift,sunX;uniform vec2 moonP;
 ...
 vec4 cloud(vec2 p,float size,float seed){
  ...
  vec3 lightDir=normalize(vec3(-.45,.8,.6));
  for(int k=0;k<11;k++){
   ...
   if(density>.001){
    ...
    lit=mix(lit,lit*vec3(1.14,.88,.64),warm*.55);
    lit=mix(vec3(.055,.078,.13)+vec3(.10,.12,.16)*sunlit,lit,day);
 ...
 color+=vec3(.86,.96,1.07)*starLight*night*night*upper*detail;
 ...
 color+=vec3(1.,.64,.23)*(exp(-sd*sd/13.)*.32+exp(-sd*sd/3.4)*.52)*sunAlpha;
 ...
 for(int i=0;i<5;i++){
  ...
  float visibility=(.32+.57*day)*detail;
 ...
 return {mesh,update,get state(){return {day:u.day.value,detail:u.detail.value,sunX:u.sunX.value,moon:u.moonP.value.toArray(),seconds:u.time.value};},dispose(){mesh.geometry.dispose();mat.dispose();}};
}
