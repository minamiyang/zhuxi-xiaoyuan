...
export function createStream(renderer,scene,camera,sun){
 ...
 const uniforms=THREE.UniformsUtils.merge([THREE.UniformsLib.lights,{uTime:{value:0},reflectionBG:{value:scene.background.clone()},mirror:{value:reflection.texture},textureMatrix:{value:new THREE.Matrix4()},eyeDir:{value:new THREE.Vector3()},sunDir:{value:sun.position.clone().normalize()},waterColor:{value:new THREE.Color().setRGB(.029,.070,.044)}}]);
 ...
 uniform float uTime;uniform vec3 reflectionBG;uniform sampler2D mirror;uniform vec3 eyeDir;uniform vec3 sunDir;uniform vec3 waterColor;varying vec3 vWorld;varying vec4 vMirror;
 #include <common>
 #include <packing>
 ...
 void main(){
  ...
  vec2 uv=vMirror.xy/vMirror.w+n.xz*.007;vec3 refl=texture2D(mirror,uv).rgb;
  vec3 ray=reflect(-e,n);vec3 sky=mix(vec3(.74,.80,.86),vec3(.13,.24,.36),smoothstep(.12,.92,ray.y));
  float skyMask=1.-smoothstep(.005,.055,length(refl-reflectionBG));refl=mix(refl,sky,skyMask);
  float shadow=getShadowMask();vec3 halfDir=normalize(sunDir+e);float spec=pow(max(dot(n,halfDir),0.),170.)*3.6;
  vec3 scatter=waterColor*(.60+.50*shadow);
  float ripple=pow(max(0.,sin(p.y*48.-p.x*13.-t*1.8+.65*sin(p.x*19.+t*.5))),18.)*.006;
  vec3 color=mix(scatter,refl,clamp(.22+fresnel*1.55,.22,.85))+vec3(1.,.91,.70)*(spec+ripple)*shadow;
  gl_FragColor=vec4(color,.88+fresnel*.10);
 }`});
 ...
 return {water,reflection,update,excluded,dirty(){dirty=true;},dispose(){reflection.dispose();geometry.dispose();mat.dispose();}};
}
