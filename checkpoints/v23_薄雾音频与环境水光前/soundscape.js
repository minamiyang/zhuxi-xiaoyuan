...
export function createSoundscape(){
 ...
 for(const [id,title]of Object.entries(titles)){const gain=ctx.createGain();gain.gain.value=0;if(id==='dog'){const soften=ctx.createBiquadFilter();soften.type='lowpass';soften.frequency.value=2300;soften.Q.value=.55;gain.connect(soften);soften.connect(master);}else gain.connect(master);tracks.set(id,{id,title,gain,target:0,buffer:null,normalization:1,source:null,loaded:false,timer:0,oneShot:id==='rooster'||id==='dog',pending:false,active:false,playCount:0});}
