...
export function createStream(renderer,scene,camera,sun){
 ...
 // Two separate light responses: a fading sun must not erase moon or sky reflection.
  ...
  refl=mix(refl,sky,.16+.16*golden);
  ...
  vec3 color=mix(scatter,refl,clamp(.28+fresnel*.85,.28,.88))
    +crestColor*(skySheen+ripple*(.40+.60*shadow))*skyEnergy
    // Broken warm sky radiance on the moving crests; keep the clear troughs cool.
    +sunColor*ripple*golden*solarEnergy*2.6*(.30+.70*shadow)
    +sunColor*spec*solarEnergy*shadow
    +moonColor*moonSpec*moonEnergy*moonShadow*(.30+.70*crest*breakup);
