...
export function sampleDay(hour){
 ...
 return {hour:h,altitude,day,high,warm,golden,sunrise,sunset,sunPower,lamps,moonPower:.64*(1-day),fog:.205*Math.exp(-(((h-6.2)/1.13)**2))*(1-smooth(7.3,8.5,h)),sunPosition,skySunPosition,exposure:mix(.97,1.03,day)};
}
const PALETTE=[
...
export function createDaylight({scene,renderer,sun,fill,hemi,ground,noAO,camera}){
 ...
 void main(){
  ...
  gl_FragColor=vec4(uFogTint,envelope*max(.08,wisps)*height*uDawnFog*(.65*11./48.));
 }`});
