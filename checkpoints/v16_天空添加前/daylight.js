...
export function sampleDay(hour){
 ...
 const sunPower=3.65*Math.pow(Math.max(0,altitude),.24)*smooth(0,.13,altitude);
 const lamps=1-smooth(-.13,.11,altitude);
 return {hour:h,altitude,day,high,warm,sunPower,lamps,moonPower:.64*(1-day),fog:.078*Math.exp(-(((h-6.35)/1.32)**2)),sunPosition:[-13*Math.cos(angle)-3.5*Math.max(0,altitude),Math.max(.12,12*altitude),6-3*Math.cos(angle)],exposure:mix(.97,1.03,day)};
}
const PALETTE=[
 [0,'#303948','#223859','#152a43'],[4.8,'#111c30','#344362','#131b2b'],
 [5.65,'#757d85','#9da8b1','#52616e'],[6.35,'#ccd1d1','#d8e1e6','#b9c5cf'],
 [8,'#d4d8d4','#d4e2ed','#c1d5e7'],[12,'#d5dce2','#cde2f5','#b8ccdf'],
 [15.5,'#d8d1c3','#d7e0e7','#b9cada'],[17.2,'#bfc6cc','#d2dfeb','#bfc4c4'],
...
export function createDaylight({scene,renderer,sun,fill,hemi,ground,noAO,camera}){
 ...
 sun.position.set(...s.sunPosition);sun.intensity=s.sunPower;sun.color.set('#fff1de').lerp(new T.Color('#ffb774'),s.warm*.88);hemi.color.set('#a7bddb').lerp(sky,s.day);hemi.groundColor.set('#655141').lerp(new T.Color('#172035'),1-s.day);hemi.intensity=mix(.48,mix(.86,.58,s.high),s.day);fill.color.set('#a7c5ff');fill.intensity=s.moonPower+.18*s.day;renderer.toneMappingExposure=s.exposure;
 ground.material.color.copy(bg).multiplyScalar(mix(.65,.36,s.day));ground.material.emissive.copy(bg).multiplyScalar(mix(.82,.42,s.day));fogUniforms.uDawnFog.value=s.fog;fogUniforms.uFogTint.value.copy(sky).lerp(new T.Color('#ecdbb8'),.25);fogUniforms.uWeatherTime.value=seconds;camera.getWorldDirection(fogUniforms.uFogViewDir.value);fogObjects.forEach(o=>o.visible=s.fog>.0001);
