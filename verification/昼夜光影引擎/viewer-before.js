...
function render(force=false){if(disposed)return;applyMotion();if(force||time-lastShadow>.10||lastShadow<0){renderer.shadowMap.needsUpdate=true;lastShadow=time;}const u=ao.ssaoMaterial.uniforms;u.cameraProjectionMatrix.value.copy(camera.projectionMatrix);u.cameraInverseProjectionMatrix.value.copy(camera.projectionMatrixInverse);u.cameraNear.value=camera.near;u.cameraFar.value=camera.far;
 // First render builds the shadow map, then the reflection can sample it.
 if(stream&&sun.shadow.map)stream.update(time,force);composer.render();}
function frame(now){raf=0;if(disposed||document.hidden)return;const dt=lastNow?Math.min((now-lastNow)/1000,.10):0;lastNow=now;if(mode==='explore'&&playing&&loaded)time+=dt;if(transition){const t=Math.min(1,(now-transition.start)/650),e=t*t*(3-2*t);camera.position.lerpVectors(transition.from,transition.to,e);controls.target.lerpVectors(transition.a,transition.b,e);camera.zoom=THREE.MathUtils.lerp(transition.z0,transition.z1,e);camera.updateProjectionMatrix();if(t===1)transition=null;}
 const changed=controls.update();if(mode==='explore'&&loaded&&(dirty||changed||transition||controls.autoRotate||playing)&&now-lastRender>30){render();dirty=false;lastRender=now;}
 if(mode==='explore'&&(playing||changed||transition||controls.autoRotate||dirty))request();}
...
function load(){if(loading||loaded)return;loading=true;status.textContent='正在展开这座小院…';new GLTFLoader().setDRACOLoader(new DRACOLoader().setDecoderPath('./node_modules/three/examples/jsm/libs/draco/gltf/')).load('./竹溪小院_动态.glb',g=>{model=g.scene;
 ...
 const convert=mat=>{const m=tuneMaterial(mat.clone(),mat.name);m.side=THREE.DoubleSide;if(moving)patchMotionMaterial(m,uniforms);return m;};o.material=Array.isArray(o.material)?o.material.map(convert):convert(o.material);
 ...
 stream=createStream(renderer,scene,camera,sun);wakes=createWakes(scene,ducks);stream.excluded.push(...wakes.objects);noAO.push(stream.water,...wakes.objects);loaded=true;loading=false;status.textContent='';window.sceneReady=true;render(true);stream.dirty();render(true);plate.classList.toggle('hidden',mode==='explore');lastNow=0;request();},p=>{if(p.total)status.textContent=`正在展开这座小院… ${Math.round(p.loaded/p.total*100)}%`;},e=>{console.error(e);loading=false;status.textContent='模型未能载入，请检查本地服务后刷新。';});}
function setMode(next){mode=next;plate.classList.toggle('hidden',next==='explore'&&loaded);$('still').classList.toggle('active',next==='still');$('explore').classList.toggle('active',next==='explore');$('hint').textContent=next==='still'?'光线追踪 · 午后暖阳':'拖动旋转 · 滚轮缩放 · 右键平移';$('motion').disabled=next==='still';lastNow=0;if(next==='explore'){load();dirty=true;request();}else{controls.autoRotate=false;$('spin').classList.remove('active');}}
function go(key){view=key;const v=views[key];plate.src='../renders/'+v.img;document.querySelectorAll('[data-view]').forEach(el=>el.classList.toggle('active',el.dataset.view===key));transition={start:performance.now(),from:camera.position.clone(),to:new THREE.Vector3(...v.p),a:controls.target.clone(),b:new THREE.Vector3(...v.t),z0:camera.zoom,z1:v.zoom};if(matchMedia('(prefers-reduced-motion: reduce)').matches)transition.start-=700;dirty=true;request();}
function setPlaying(value){playing=value;$('motion').textContent=value?'暂停动态':'继续动态';$('motion').setAttribute('aria-pressed',String(!value));lastNow=0;dirty=true;request();}
$('still').onclick=()=>setMode('still');$('explore').onclick=()=>setMode('explore');$('motion').onclick=()=>setPlaying(!playing);document.querySelectorAll('[data-view]').forEach(el=>el.onclick=()=>go(el.dataset.view));$('spin').onclick=()=>{setMode('explore');controls.autoRotate=!controls.autoRotate;$('spin').classList.toggle('active',controls.autoRotate);request();};$('save').onclick=()=>{const a=document.createElement('a');a.download='竹溪小院.png';if(mode==='still')a.href=plate.src;else{render(true);a.href=renderer.domElement.toDataURL('image/png');}a.click();};
function resize(){const aspect=innerWidth/innerHeight,h=Math.max(13.7,16.8/aspect);camera.left=-h*aspect/2;camera.right=h*aspect/2;camera.top=h/2;camera.bottom=-h/2;camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight);composer.setSize(innerWidth,innerHeight);stream?.dirty();dirty=true;if(loaded)render(true);request();}
window.addEventListener('resize',resize);document.addEventListener('visibilitychange',()=>{lastNow=0;if(document.hidden){if(raf)cancelAnimationFrame(raf);raf=0;}else{dirty=true;request();}});renderer.domElement.addEventListener('webglcontextlost',e=>{e.preventDefault();status.textContent='图形上下文暂时中断，请刷新恢复。';});window.addEventListener('pagehide',()=>{disposed=true;cancelAnimationFrame(raf);controls.dispose();composer.dispose();stream?.dispose();env.dispose();renderer.dispose();});
window.viewer={setMode,go,setPlaying,setTime(t){setPlaying(false);time=t;render(true);},duckPose,peckAngle,windOffset,riverCenter,riverHalfWidth,get state(){return {mode,view,loaded,playing,time,meshes:renderer.info.memory.geometries,zoom:camera.zoom,actors:actors.map(a=>({name:a.object.name,type:a.type,index:a.index,position:a.object.position.toArray(),rotation:a.object.rotation.toArray(),peck:a.uniforms.uPeck.value})),movingGroups:motionUniforms.length};},scene,camera,controls,renderer,henPose,hen:()=>rebuiltHen,stream:()=>stream};resize();setPlaying(playing);setMode('explore');
