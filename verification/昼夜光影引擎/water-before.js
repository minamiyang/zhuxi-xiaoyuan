...
export function createStream(renderer,scene,camera,sun){
 ...
 const uniforms=THREE.UniformsUtils.merge([THREE.UniformsLib.lights,{uTime:{value:0},reflectionBG:{value:scene.background.clone()},mirror:{value:reflection.texture},textureMatrix:{value:new THREE.Matrix4()},eyeDir:{value:new THREE.Vector3()},sunDir:{value:sun.position.clone().normalize()},waterColor:{value:new THREE.Color().setRGB(.064,.12,.072)}}]);
 ...
 void main(){
  ...
  vec3 color=mix(scatter,refl,clamp(.08+fresnel*1.55,.08,.70))+vec3(1.,.91,.70)*(spec+ripple)*shadow;
  gl_FragColor=vec4(color,.60+fresnel*.25);
 }`});
